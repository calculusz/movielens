...
from src.loadMovieLens import loadMovieLensTrain
from src.loadMovieLens import loadMovieLensTest
from src.loadMovieLens import loadResult
...
##==================================================================
def getAllUserRating(fileTrain='ua.base', fileTest='ua.test', fileResult='result.txt', similarity=sim_pearson):
    ...
    for userid in prefer2:             #test集中每个用户
        for item in prefer2[userid]:   #对于test集合中每一个项目用base数据集,CF预测评分
            ...
            # itemint=int(item)
            beforesort.setdefault(userid,{})
            beforesort[userid][item]=rating
            # beforesort.append([useridint,itemint,rating])
            file.write('%s\\%s\\%s\n'%(userid, item, rating))
            inAllnum = inAllnum +1

    file.close()
    # beforesort=loadResult()

    # sortresults=sorted(beforesort,key=itemgetter(0,1))
    # print("u"+userid+"item"+item)
    # temp=[abs(beforesort[userid][item]-prefer2[userid][item]) for item in beforesort]
    # sortresults=sorted(beforesort,)
    subsum=0
    lenssub=0
    for userid in beforesort:
        subsum+=sum([abs(beforesort[userid][item]-prefer2[userid][item]) for item in beforesort[userid]])
        lenssub+=len(beforesort[userid])
    MAE=subsum/lenssub
    print("-------------Completed!!-----------",inAllnum,MAE)


############    主程序   ##############
if __name__ == "__main__":
    print("\n--------------基于MovieLens的推荐系统 运行中... -----------\n")
    # getAllUserRating('ua.base', 'ua.test', 'result.txt')
    data={}
    # data=loadResult()
    f=open("resultspickle.dat",'rb')
    # pickle.dump(data,f)
    data=pickle.load(f)
    f.close()
